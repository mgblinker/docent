...
// Module identity from the file path: repo name, plus (for the Backend
// modular monolith specifically) the module folder right under it.
export function moduleFromPath(filePath) {
  const rel = path.relative(PROJECTS_ROOT, filePath);
  const segments = rel.split(path.sep);
  const repo = segments[0];
  if (repo === "Processity.Backend" && segments.length > 1) {
    return `Backend/${segments[1]}`;
  }
  return repo.replace(/^Processity\./, "");
}
